<template>
  <div ref="modelWrap" class="model-wrap" :style="{ width: `${currModel.width}px`, height: `${currModel.height}px` }">
    <div class="message" ref="message" :style="{ bottom: currModel.offset }">
      {{ message }}
    </div>
    <canvas class="live2d" id="live2d" @click="click2dCanvas" :width="currModel.width" :height="currModel.height"></canvas>
  </div>
</template>
...
import "./style.scss";
import { loadjs } from "../../../public/live2d/js/utils";
...
export default {
  ...
  methods: {
    // 初始化 live2d
    initLoadLive2d() {
      loadjs(`/live2d/js/live2d.js`).then(() => {
    ...
    getJson() {
      axios.get(`/live2d/modules/ntw20_2301/ntw20_2301.model.json`).then(() => {
        setTimeout(() => {
          this.loadlive2d("live2d", `/live2d/modules/ntw20_2301/ntw20_2301.model.json`);
        }, 100);
      });
    },
